...
export function createPlanTool(options: PlanToolOptions = {}): Tool {
  const name = options.name ?? 'update_plan';
  ...
  return {
    ...
    async execute(inputText, context) {
      return formatPlanResult(parsePlanInput(inputText, name), context, name);
    },
...
function parsePlanInput(inputText: string, toolName: string): ParsedPlanInput {
  let parsed: unknown;
  ...
  if (plan.length === 0) {
    throw new Error(`${toolName} input requires at least one plan step`);
  }

  const inProgressCount = plan.filter((entry) => entry.status === 'in_progress').length;

  if (inProgressCount > 1) {
    throw new Error(`${toolName} input allows at most one in_progress step`);
  }
